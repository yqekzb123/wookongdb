...
static RangeDesc* kvrange_scan_get_next(KVEngineIteratorInterface **engine_it, TupleKeySlice cmp_key);
static bool rangekey_scan_get_next(KVEngineIteratorInterface **engine_it, TupleKeySlice cmp_key, RangeSatistics* statistics);
...
static bool scan_get_middle_key(KVEngineIteratorInterface **engine_it, TupleKeySlice cmp_key, Dataslice *key, Dataslice *value, Size all_size);

static bool
key_cmp(TupleKeySlice rocksdb_key, TupleKeySlice target_key)
{
    return memcmp(rocksdb_key.data, target_key.data, get_min(rocksdb_key.len, target_key.len)) <= 0 ? true : false;
}
...
get_range(TupleKeySlice rocksdb_key, TupleKeySlice target_key)
{
    ...
    return rocksdb_key.data->indexOid < target_key.data->indexOid ? true : false;
}
...
{
	if ((*engine_it)->is_valid(*engine_it))
	{
		...
		int r = memcmp(tempkey.data, cmp_key.data, get_min(tempkey.len, cmp_key.len));
		if (r == 0)
		{
...
kvengine_process_scan_all_range(void)
{
    ...
    KVEngineTransactionInterface *txn = engine->create_txn(engine, 2, true, t);
	KVScanDesc desc = init_kv_scan(true, txn);
    ...
    t = GetTimeStamp();
    txn->commit_and_destroy(txn, 2, t);
	return rangelist;
}

RangeDesc*
kvrange_scan_get_next(KVEngineIteratorInterface **engine_it, TupleKeySlice cmp_key)
{
	if ((*engine_it)->is_valid(*engine_it))
	{
		TupleKeySlice tempkey = {NULL, 0};
...
void kvengine_process_scan_one_range_all_key(TupleKeySlice startkey, TupleKeySlice endkey, RangeID rangeid, RangeSatistics *Rangestatis, int gxid) {
    uint64_t t = GetTimeStamp();
    KVEngineTransactionInterface *txn = engine->create_txn(engine, gxid, true, t);
	KVScanDesc desc = init_kv_scan(true, txn);
...
kvengine_process_init_statistics_req(RequestHeader* req)
{
    ...
    kvengine_process_scan_one_range_all_key(start_key, end_key, range_req->rangeid, rangeSatistics, req->gxid);
...
kvengine_process_rangescan_req(RequestHeader* req)
{
    ...
    KVEngineTransactionInterface *txn = engine->create_txn(engine, range_req->header.gxid, true, range_req->header.start_ts);
	KVScanDesc desc = init_kv_scan(true, txn);
